<?php

namespace App\Command\Tema;

use App\Command\BaseApiCommand;
use App\Entity\IConnection;
use App\Repository\ApiFetchErrorRepository;
use App\Repository\SourceAuthRepository;
use App\Repository\Tema\FkDokInfoRepository;
use Symfony\Component\Console\Attribute\AsCommand;
use Symfony\Component\Console\Input\InputArgument;
use Symfony\Component\Console\Style\SymfonyStyle;

#[AsCommand(
    name: 'tema:fk:dokinfo',
    description: 'Pobiera informacje o dokumentach dla roku. Parametry: nazwa api (opcja), dateFrom [domyślnie bieżący rok]',
)]
class FkDokInfoCommand extends BaseApiCommand
{
    private $dokinfo;
    protected $producerName = 'Tema';

    public function __construct(FkDokInfoRepository $dokinfo, SourceAuthRepository $apiAuthRepo, ApiFetchErrorRepository $errorRepo)
    {
        parent::__construct($apiAuthRepo, $errorRepo);
        $this->dokinfo = $dokinfo;
    }
    
    protected function configure(): void
    {
        $this
            ->addArgument('api', InputArgument::OPTIONAL, 'Nazwa api (ALL lub brak nazwy dla wszystkich)', 'ALL')
            ->addArgument('dateFrom', InputArgument::OPTIONAL, 'Data od [domyślnie bieżący rok]', date('Y-01-01'))
        ;
    }

    protected function fetch(IConnection $api, SymfonyStyle &$io)
    {
        $this->dokinfo->setSource($api);
        $this->dokinfo->setDateFrom($this->dateFrom);
        $io->text(sprintf("Pobieranie informacji o dokumentach dla roku %s", date('Y', strtotime($this->dateFrom))));

        $fetchedRows = $this->dokinfo->fetch();
        $io->info(sprintf("Pobrano %s rekordów", $fetchedRows['fetched']));
        unset($fetchedRows);
    }

    protected function clearTable()
    {
        $this->dokinfo->clearTable();
    }
}