...
class PzItemRepository extends IApiRepository
{
    ...
    {
        $docIds = array_unique(array_map(function($item) { return $item['pz_id']; }, $this->fetchResult));
        if (count($docIds)) {
            $placeholders = implode(',', array_fill(0, count($docIds), '?'));
            $this->db->executeStatement("DELETE FROM $this->table WHERE source = ? AND pz_id IN ($placeholders)", array_merge([$this->source->getName()], $docIds), array_merge([ParameterType::STRING], array_fill(0, count($docIds), ParameterType::STRING)));
        }
    }
